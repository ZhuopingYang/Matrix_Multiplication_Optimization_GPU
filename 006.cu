/**
 * Vectorize access, 9782.339 GOPs
*/
...
__global__ void matrix_multiplication_gpu(float *A, float *B, float *C, int N){

    int inner_row_A = threadIdx.x / (BLK2 / 4);
    int inner_col_A = threadIdx.x % (BLK2 / 4);

    int inner_row_B = threadIdx.x / (BLK1 / 4);
    int inner_col_B = threadIdx.x % (BLK1 / 4);
    ...
    for(int i = 0; i < N; i += BLK2){
        ...
        for(int j = 0; j < BLK0; j += TM0 * 4){
            reinterpret_cast<float4 *>(&buf_a[(j + inner_row_A) * BLK2 + inner_col_A * 4])[0] = \
                                 reinterpret_cast<float4 *>(&A[(blockIdx.y * BLK0 + inner_row_A + j) * N + inner_col_A * 4 + i])[0];
        }
        
        for(int j = 0; j < BLK2; j += 4){
            reinterpret_cast<float4 *>(&buf_b[(j + inner_row_B) * BLK1 + inner_col_B * 4])[0] = \
                                 reinterpret_cast<float4 *>(&B[(j + inner_row_B + i) * N + blockIdx.x * BLK1 + inner_col_B * 4])[0];
        }
        ...
        for(int j = 0; j < BLK2; ++j){
            for(int k = 0; k < TM0; ++k){
                regA[k] = buf_a[out_row * TM0 * BLK2 + j + k * BLK2];
                regB[k] = buf_b[(j) * BLK1 + out_col * TM1 + k];
            }


            for(int k = 0; k < TM0; ++k){
                for(int l = 0; l < TM1; ++l){
                    thread_res[k * TM1 + l] += regA[k] * regB[l];
                }
            }
        }
    ...
    for(int i = 0; i < TM0; ++i){
        for(int j = 0; j < TM1; j += 4){
            reinterpret_cast<float4 *>(&C[(blockIdx.y * BLK0 + out_row * TM0 + i) * N + blockIdx.x * BLK1 + out_col * TM1 + j])[0] = \
                                 reinterpret_cast<float4 *>(&thread_res[i * TM1 + j])[0];
        }
    }
